	...
	if ($event['type'] == 'message') 
	{
		if($event['message']['type'] == 'text')
		{
			...
			if($text[0] == "@บอท"){
				...
				if($text[1]." ".$text[2] == "ขอรายชื่อนิสิต รหัส"){
					$reply_message = mySQL_selectAll('http://bot.kantit.com/json_select_users.php?sid='.$text[3]);
				}
				if($text[1]." ".$text[2]." ".$text[3] == "ขอรหัส FTP ของ"){
					$reply_message = mySQL_selectFTP('http://bot.kantit.com/json_select_ftp.php?sid='.$text[4]);					
				}	
			}
